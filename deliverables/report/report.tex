...
\emph{structured LLM payloads} (not freeform generation),
...
\textbf{Leakage prevention.}  Feature extraction and LLM prompting
use only \texttt{train\_target} samples.  The benign development set
(\texttt{benign\_dev}) gates rule acceptance but is never used for
feature selection.  Final metrics are computed exclusively on
\texttt{test\_target} and \texttt{benign\_test}, which are unseen
during all prior stages.  No baseline receives information from the
test split.
...
\begin{tikzpicture}[
  ...
  \node[note, below=0.15cm of sel] {TF-IDF\\top-$k$=30};
  \node[note, below=0.15cm of eva] {TPR, FPR\\$F_1$, spec.};

  % LLM callout
  \node[note, above=0.3cm of gen] {LLM JSON\\$\rightarrow$ renderer};
\end{tikzpicture}
\caption{%
...
Static PE features are extracted from \texttt{train\_target} samples:
printable ASCII and wide strings, PE import names (DLL and function),
section names, and PE metadata (timestamps, entry-point
characteristics).  No dynamic or behavioral features are used.

\subsection{Stage 4: Select}
Discriminative features are ranked using TF-IDF
weighting~\cite{salton1988tfidf} computed on the training malware
corpus.  The top $k=30$ features per family are retained
(\texttt{top\_k\_features}=30).  This ranking uses only
\texttt{train\_target}; no benign data influences feature selection.
...
valid YARA source.  The LLM never writes YARA syntax directly; it only
selects features and logical structure.
...
The default configuration records \texttt{temperature\_config}=1.0,
but GPT-5-family requests are not described as relying on an explicit
temperature override in this report.
...
\sysname achieves the highest mean~$F_1$ (0.4240) and the highest
...
\topstrings.  In operational deployment, a rule with zero benign FPR
generates fewer analyst alerts than one with FPR=0.006, which may
justify the lower recall if the analyst workload budget is constrained.
...
% ============================================================
\section{Saved Results and Replay}
\label{sec:reproducibility}

\subsection{Saved Result Bundle}
All metrics reported in this paper are drawn from the saved result
bundle under \path{artifacts/final_motif_audit/}.  The bundle includes:
\begin{itemize}[nosep,leftmargin=1.2em]
  \item Per-family evaluation CSVs with TPR, FPR, $F_1$, specificity,
    and complexity (\path{results_per_family.csv},
    \path{comparison_rows.csv}).
  \item Aggregate summary JSONs (\texttt{primary\_summary.json},
    \path{comparison_summary.json}, \path{replay_summary.json}).
  \item Significance and failure-analysis artifacts
    (\path{significance_vs_primary.json},
    \path{failure_analysis.json}).
  \item A sanitized \path{run_manifest.json} with run metadata and
    artifact hashes.
\end{itemize}

\subsection{Run Metadata}
\begin{itemize}[nosep,leftmargin=1.2em]
  \item \textbf{Model:} gpt-5-mini (OpenAI backend)
  \item \textbf{Platform:} macOS, Python 3.12.2
  \item \textbf{Dependencies:} yara-python 4.5.1,
    scikit-learn 1.6.1~\cite{pedregosa2011sklearn}, openai 1.61.1
  \item \textbf{Seed:} 42 (splits and significance tests)
\end{itemize}

\subsection{Replay Verification}
A full replay of the \sysname pipeline was executed to assess
reproducibility.  \cref{tab:replay} compares the primary and replay
runs.

\begin{table}[H]
\centering
\caption{Primary vs.\ replay run comparison.}
\label{tab:replay}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Metric} & \textbf{Primary} & \textbf{Replay} \\
\midrule
Families OK       & 128\,/\,138 & 128\,/\,138 \\
Mean $F_1$        & 0.4240      & 0.4278 \\
Mean TPR          & 0.4043      & 0.4043 \\
Mean FPR\textsubscript{ben} & 0.0000 & 0.0000 \\
Mean Specificity  & 0.9988      & 0.9990 \\
\bottomrule
\end{tabular}
\end{table}

The replay rerun is close but not identical.  The same 128 families
are accepted in both runs, and mean TPR is identical (0.4043).  Mean
$F_1$ changes slightly (0.4240 vs.\ 0.4278), so we report replay as a
check on the saved outputs rather than as exact equality.
...
\begin{itemize}[nosep,leftmargin=1.2em]
  \item Highest mean $F_1$ in the executed comparison, but not a
