...
\path{deliverables/examples/nanolocker.yar}.
...
Immediate follow-up directions are clear from the current results:
improve coverage on the 10 rejected families, add feature paths that
help with packed or low-string samples, and test the same pipeline on
additional malware corpora beyond \motif.  A second track is to explore
whether stronger acceptance logic can preserve the current benign-FPR
behavior while recovering some of the recall gap against the
byte-oriented baselines.
...
The full rule text is reproduced from
\texttt{deliverables/examples/nanolocker.yar}.
